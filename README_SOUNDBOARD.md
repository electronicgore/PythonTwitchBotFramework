...
* `soundbank_default_price` (default: `50`): default price of playing a sound, in terms of bot currency (like twitch points, but internal to the bot). Set to `0` if you do not want to use this.
...
* `soundbank_gain` (default: `0`): global volume level for all sounds, in dB.
* `soundbank_cooldown` (default: `15`): cooldown for playing sounds (in seconds). 


# Adding sounds
There are two ways to add sounds to the soundboard: automatic and manual. Most widespread sound formats should work.
...
E.g., file `<botfolder>/sounds/wow.mp3` will be named `wow` and can be then played with `!sb wow`. 
...
* `q` -- *quiet*, to suppress detailed reporting in the bot window (not in chat).
...
To play a sound with name `sndid`, use command `!sb sndid`. It is not currently possible to add sounds as first-level commands (e.g., `!sndid`) without manually creating a custom command.

If you would like to list all sounds currently present in the soundbank, use `!gensblist`. Note that this will not output the list in chat, but rather create a text file in your sounds folder (to avoid problems with large soundbanks). The list will contain all sounds and their current prices. The list is unsorted afaik.
...
Changing the filename is not possible, because it would be too painful to parse with regexp. But it is possible to change the sound name. All other options are same as in [Adding sounds: manual](#adding-sounds-manual)
